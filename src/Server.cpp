...
void Server::updateAIEntities(float deltaTime) {
	...
	size_t entityListSize = m_aiEntities.size() * sizeof(AIEntity);

	char* pBuff = (char*)malloc(entityListSize);
	//memcpy(pBuff, (const char*)packetNumber, sizeof(unsigned int));
	memcpy(pBuff, (const char*)m_aiEntities.data(), entityListSize);
	broadcastFaultyData(pBuff, entityListSize + sizeof(unsigned int));
	free(pBuff);
